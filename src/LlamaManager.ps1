...
$Script:ModelSearchPaths = @($Script:ModelsPath, $Script:BasePath)
...
function Invoke-SimpleWizard {
    <#
    .SYNOPSIS
        Wizard simple que solo pide la ruta del modelo GGUF y ejecuta la herramienta.
    #>
    param([string]$ExePath, [string]$ToolName)

    Write-Host ""
    Show-Separator "WIZARD SIMPLE: $ToolName" "Magenta"
    Write-Color "  Ejecucion rapida solo con modelo" "DarkGray"
...
function Start-MainLoop {
    ...
    while ($true) {
        ...
        $mainChoice = Show-Menu -Title "MENU PRINCIPAL" -Options @(
            ...
            "[2] Ejecutar herramienta (Modo Simple)"
            "[3] Perfiles guardados (Presets)"
            "[4] Buscar modelos .gguf"
            "[5] Historial de comandos",
            "[6] Chat rapido (Ultimo modelo)",
            "[7] Monitor de Servidor en Background",
            "[8] Analizar Documento de Texto (Lotes)",
            "[9] Descargar modelo de HuggingFace",
            "[10] Leer Metadata GGUF",
            "[11] Actualizar llama.cpp",
            "[12] Info del sistema",
            "[13] Salir"
        ) -Descriptions @(
            "Seleccionar y configurar una herramienta"
            "Ejecucion rapida solo pidiendo el modelo"
        ...
        switch ($mainChoice) {
            ...
            1 {
                # --- Ejecutar herramienta (Modo Simple) ---
                $tool = Show-ToolSelectionMenu
                if (-not $tool) { continue }
                $commandInfo = Invoke-SimpleWizard -ExePath $tool.Path -ToolName $tool.BaseName
                if ($commandInfo) {
                    Show-PostWizardMenu -CommandInfo $commandInfo
                }
            }
